...
'filtered_counts_combined.csv' that has low expressed genes removed. A gene will be discraded, if none of the
...
2) Aligned bam files are assigned sample names instead of treatment and replicate names
3) Run genomicfeatures.R to counts reads per gene
4) Included a python script low-expressed genes to filter genes based on treatment groups 
5) User can provide cutoff for discarding low-expressed genes
6) Can now work on full path for rnaseqcripts folder
7) Uses python3
'''
